...
const isRgbModeActive = ref(false);
...
export function useRgbMode() {
    const startRgbAnimation = () => {
        ...
        const animate = () => {
            ...
            currentHue.value = (currentHue.value + 0.5) % 360;

            // Converte HSL para RGB
            const rgb = hslToRgb(currentHue.value, 85, 60);
            ...
            const brightRgb = hslToRgb(currentHue.value, 90, 70);
            const dimRgb = hslToRgb(currentHue.value, 80, 40);
    ...
    const toggleRgbMode = () => {
        isRgbModeActive.value = !isRgbModeActive.value;
        ...
        localStorage.setItem('rgb-mode-active', isRgbModeActive.value.toString());
    };
    ...
    const savedState = localStorage.getItem('rgb-mode-active');
    if (savedState === 'true') {
        isRgbModeActive.value = true;
        startRgbAnimation();
    }
