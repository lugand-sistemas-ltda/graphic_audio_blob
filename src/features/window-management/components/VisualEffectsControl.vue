<template>
    <div class="visual-effects-control" v-draggable="{ id: 'visual-effects-control', handle: '.vfx-header' }">
        <div class="vfx-header">
            <span class="vfx-title">[ VISUAL EFFECTS ]</span>
            <button class="collapse-toggle" @click="toggleExpanded" :aria-label="isExpanded ? 'Collapse' : 'Expand'">
                {{ isExpanded ? '−' : '+' }}
            </button>
        </div>

        <div v-if="isExpanded" class="vfx-content">
...
// Obtém windowId do contexto
const windowId = inject<WindowId>('windowId', 'main')
...
useVisibilityReload({
    selector: '.visual-effects-control',
    onVisible: reloadState
})

const { state } = useGlobalState()

const windowConfig = computed(() => state.windows[windowId])
...
const toggleEffect = (effect: VisualEffect) => {
    toggleWindowEffect(windowId, effect)
}
</script>
